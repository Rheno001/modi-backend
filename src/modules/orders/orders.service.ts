...
export const verifyOrder = async (reference: string) => {
    // 1. Find the order
    ...
    const result = await prisma.$transaction(async (tx) => {
        ...
        if (!updatedOrder) throw new Error('Order not found during transaction');
...
export const scanTicket = async (uniqueCode: string, scannedById: string) => {
